...
pub fn NotificationsInboxPage() -> impl IntoView {
    ...
    // Trigger signal bumped after bulk operations so stats refetch.
        ...
        spawn_local_scoped(async move {
            match mark_all_notifications_read().await {
                Ok(count) => {
                    ...
                    // Bump the trigger so stats resources refetch.
                    stats_trigger.update(|n| *n += 1);
                    // Clear optimistic overrides — the server is now the
                    // source of truth and the stats query will reflect it.
