...
//! - **Inbox routes** — Auth-gated `/notifications` routes for the full inbox page.
//!   [Get started](#mount-inbox-routes)
//! - **Lazy inbox route** — Code-split inbox leaf for `cargo leptos --split` hosts.
//!   [Get started](#lazy-routes)
//!
//! # Getting started
//!
...
//! This crate submits it to `uf_integrations::ShellNotificationBellContribution`. Hosts
//! that enable `offering-notifications` / `full` and depend on `uf-notifications` get
//! the bell via inventory. Call `uf_integrations::provide_shell_notification_bell` only
//! when you need a host-specific override.
//!
...
//! ```rust,ignore
//! use leptos::prelude::*;
//! use uf_integrations::HostNotificationBell;
//! use uf_notifications::ensure_notification_bell_linked;
//!
//! // Link inventory once at App() root (or rely on NotificationsRoutes mount):
//! ensure_notification_bell_linked();
//! // HostNotificationBell in UnifiedFieldAppBar renders NotificationBell via inventory.
//! let _slot = view! { <HostNotificationBell /> };
//! ```
//!
