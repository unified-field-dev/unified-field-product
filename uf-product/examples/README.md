...
| [`app_route_paths`](app_route_paths.rs) | Bare route discovery via raw inventory | `cargo run -p uf-product --example app_route_paths --features ssr` | Prints registered paths including `/example` | Prefer `uf_app_registration` for zone apps |
