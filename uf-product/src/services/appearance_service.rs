...
pub async fn get_my_appearance() -> Result<AppearanceData, ServerFnError> {
    #[cfg(feature = "ssr")]
    {
        ...
        let existing = UserAppearance::query(&v, valence::use_!(r"In **services**, we **list User Appearance** so the product can show or process the matching set for this workflow. Callers allowed for **services** use the list; it is not a public dump of every field to anonymous visitors."))
        ...
        let row = match existing {
            ...
            None => {
                ...
                UserAppearance::create(new_row, &v, valence::use_!(r"When **services** needs to persist work, we **save User Appearance** so the next step in that feature can continue with the latest values. People and services allowed for **services** use this data for that workflow—not as a general export of unrelated personal fields."))
                    .await
...
pub async fn save_my_appearance(
    ...
    #[cfg(feature = "ssr")]
    {
        ...
        let row = UserAppearance::query(&v, valence::use_!(r"In **services**, we **list User Appearance** so the product can show or process the matching set for this workflow. Callers allowed for **services** use the list; it is not a public dump of every field to anonymous visitors."))
        ...
        let mut mutable = row
            .get_mutable(&v, valence::use_!(r"In **services**, we **update this data** so later steps see the latest values for this workflow. Callers allowed for **services** use the updated data; this is not a public export of unrelated fields."))
